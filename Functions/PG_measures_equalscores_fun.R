...
append_columns_PG_equalscores <- function(df) {
  start_time <- Sys.time()
  ...
  ISEI_scores_equal <- SSND_ISEI_scores_equal[names(df)]
  USPrest_scores_equal <- SSND_USPrest_scores_equal[names(df)]
  ...
  average_prestige_ISEI <-
    map_dbl(t_df, ~ mean((ISEI_scores_equal * .x)[(ISEI_scores_equal * .x != 0)]))
  average_prestige_USPrest <-
    map_dbl(t_df, ~ mean((USPrest_scores_equal * .x)[(USPrest_scores_equal * .x != 0)]))
  range_prestige_ISEI <-
    map_dbl(t_df, ~ diff(range((
      ISEI_scores_equal * .x
    )[(ISEI_scores_equal * .x != 0)])))
  range_prestige_USPrest <-
    map_dbl(t_df, ~ diff(range((
      USPrest_scores_equal * .x
    )[(USPrest_scores_equal * .x != 0)])))
  min_prestige_ISEI <- map_dbl(t_df, ~ min((ISEI_scores_equal * .x)[(ISEI_scores_equal * .x != 0)]))
  min_prestige_USPrest <- map_dbl(t_df, ~ min((USPrest_scores_equal * .x)[(USPrest_scores_equal * .x != 0)]))
  max_prestige_ISEI <- map_dbl(t_df, ~ max((ISEI_scores_equal * .x)[(ISEI_scores_equal * .x != 0)]))
  max_prestige_USPrest <- map_dbl(t_df, ~ max((USPrest_scores_equal * .x)[(USPrest_scores_equal * .x != 0)]))
  ...
  ISEI_USPrest_measures_NA[all_0s, c("average_prestige_ISEI_NA", "average_prestige_USPrest_NA", 
                                     "range_prestige_ISEI_NA", "range_prestige_USPrest_NA",
                                     "min_prestige_ISEI_NA", "min_prestige_USPrest_NA",
                                     "max_prestige_ISEI_NA", "max_prestige_USPrest_NA")] <- NA
  ISEI_USPrest_measures_RI[all_0s, c("average_prestige_ISEI_RI", "average_prestige_USPrest_RI", 
                                     "range_prestige_ISEI_RI", "range_prestige_USPrest_RI",
                                     "min_prestige_ISEI_RI", "min_prestige_USPrest_RI",
                                     "max_prestige_ISEI_RI", "max_prestige_USPrest_RI")] <- NA
  ...
  if (intercept_min_total_ISEI < min(SSND_ISEI_scores_equal)) {
    intercept_min_total_ISEI <- min(SSND_ISEI_scores_equal)
  }
  if (intercept_min_total_USPrest < min(SSND_USPrest_scores_equal)) {
    intercept_min_total_USPrest <- min(SSND_USPrest_scores_equal)
  }
  if (intercept_max_total_ISEI > max(SSND_ISEI_scores_equal)) {
    intercept_max_total_ISEI <- max(SSND_ISEI_scores_equal)
  }
  if (intercept_max_total_USPrest > max(SSND_USPrest_scores_equal)) {
    intercept_max_total_USPrest <- max(SSND_USPrest_scores_equal)
  }
  ...
  ISEI_USPrest_measures_NA_complete$PCA_scores_NA_ISEI <- 
    princomp(ISEI_USPrest_measures_NA_complete[, c("total_occupation", "range_prestige_ISEI_NA", "max_prestige_ISEI_NA")], cor = TRUE, fix_sign = TRUE)$scores[, 1]
  ISEI_USPrest_measures_NA_complete$PCA_scores_NA_USPrest <- 
    princomp(ISEI_USPrest_measures_NA_complete[, c("total_occupation", "range_prestige_USPrest_NA", "max_prestige_USPrest_NA")], cor = TRUE, fix_sign = TRUE)$scores[, 1]
  ISEI_USPrest_measures_RI$PCA_scores_RI_ISEI <- 
    princomp(ISEI_USPrest_measures_RI[, c("total_occupation", "range_prestige_ISEI_RI", "max_prestige_ISEI_RI")], cor = TRUE, fix_sign = TRUE)$scores[, 1]
  ISEI_USPrest_measures_RI$PCA_scores_RI_USPrest <- 
    princomp(ISEI_USPrest_measures_RI[, c("total_occupation", "range_prestige_USPrest_RI", "max_prestige_USPrest_RI")], cor = TRUE, fix_sign = TRUE)$scores[, 1]
  ...
  ISEI_USPrest_measures_NA <- left_join(ISEI_USPrest_measures_NA, ISEI_USPrest_measures_NA_complete, by = "id")
  ...
  for (i in 1:length(class_props_merged)) {
    df_class_diversity <- df_class_diversity %>%
      mutate(!!sym(class_props_merged[i]) := (!!sym(class_vars_merged[i])/total_occupation)^2)
  }
  df_class_diversity <- df_class_diversity %>%
    mutate(class_diversity_raw = 1 - rowSums(pick(class_props_merged)),
           class_diversity_std = class_diversity_raw/(1-1/4)) %>%
    dplyr::select(id, class_diversity_raw, class_diversity_std, total_occupation, sum_Higher)
  ...
  df_class_diversity_complete$PCA_scores_Class <- 
    princomp(df_class_diversity_complete[, c("total_occupation", "sum_Higher", "class_diversity_raw")], cor = TRUE, fix_sign = TRUE)$scores[, 1]
  df_class_diversity_complete <- df_class_diversity_complete %>%
    dplyr::select(id, class_diversity_raw, class_diversity_std, PCA_scores_Class)
  df <- left_join(df, df_class_diversity_complete, by = "id") %>%
    relocate(class_diversity_raw, class_diversity_std, PCA_scores_Class, .after = sum_Manual)
  end_time <- Sys.time()
  list(occups_present = df_occups, df = df, simulation_time = end_time-start_time)
}
